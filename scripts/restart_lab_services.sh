...
if ! require_cmd docker; then
  err "Docker is not installed or not in PATH. Ejecuta cloud-init o install_lab_prereqs.sh"
  exit 1
fi
systemctl start docker >/dev/null 2>&1 || true

# curl para descubrir IP
...
if [[ -n "$PUBLIC_IP_OVERRIDE" ]]; then
  ...
  if [[ -z "$PUBIP" || "$PUBIP" == "null" ]]; then PUBIP="$(get_public_ip_egress_ifconfig || true)"; fi
  if [[ -z "$PUBIP" || "$PUBIP" == "null" ]]; then PUBIP="$(get_public_ip_egress_ipify || true)"; fi
fi
[[ -z "$PUBIP" || "$PUBIP" == "null" ]] && PUBIP="<no-public-ip>"

ADV_IP="$VM_PRIV_IP"; [[ "$PUBIP" != "<no-public-ip>" ]] && ADV_IP="$PUBIP"
...
docker run -d --name redis --network labnet -p 6379:6379 --restart unless-stopped redis:7.2

docker rm -f redisinsight >/dev/null 2>&1 || true
docker run -d --name redisinsight --network labnet -p 8001:8001 --restart unless-stopped redislabs/redisinsight:1.14.0

# HBase
docker rm -f hbase >/dev/null 2>&1 || true
docker run -d --name hbase --network labnet \
  -p 2181:2181 -p 16000:16000 -p 16010:16010 -p 16030:16030 -p 9090:9090 \
  --restart unless-stopped harisekhon/hbase:latest

# Kafka (dual listeners) + Kafka UI
...
printf "%-22s %-8s %s\n" "Kafka bootstrap"     "29092"  "${ADV_IP}:29092"
